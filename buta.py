...
async def logUserBot():
    load_dotenv()
    api_id = int(25429269)
    api_hash = "cc518836f6ba0a735bd5c270f586f512"
    phone_number = "51907885477"
    ...
    await client.send_message("@spam202497", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4214825572")
    ...
    while True:
        ...
        try:
            await client.send_message("@spam202497", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["spam2024","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spam202497", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spam202497", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(10)
                        if j==1: break
            await client.send_message("@spam202497", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
